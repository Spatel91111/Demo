    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        case .suggestions:
            if let item = item as? SearchViewModelSuggestionItem {
                if let cell = tableView.dequeueReusableCell(with: SearchCell.self, for: indexPath) {
                    cell.productImage?.setImage(fromURL: item.products[indexPath.row].thumbNail)
                    ...
                    if item.products[indexPath.row].hasSpecialPrice {
                        ...
                        cell.productName.attributedText = attributeString
                    } else {
                        cell.productName.text = item.products[indexPath.row].productName.html2String + "\n" + item.products[indexPath.row].price
                    }
