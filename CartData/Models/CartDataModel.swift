...
public struct CartDataModel {
    ...
    private struct SerializationKeys {
        ...
        static let banner_img = "banner_img"
    }
    ...
    public init(json: JSON) {
        ...
        banner_img = json[SerializationKeys.banner_img].stringValue
        ...
        if let items = json[SerializationKeys.totalsData].array {
            ...
            totalsData = array.filter{ $0.unformattedValue != 0 || $0.title == "Order Total".localized}
        }
        if let items = json[SerializationKeys.giftcard].array { giftData = items.map { GiftData(json: $0) } }
