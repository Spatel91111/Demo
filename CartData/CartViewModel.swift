...
class CartViewModel: NSObject {
    ...
    enum CartApi {
        ...
        case updateCart
        case emptyCart
        case applyGiftCardCoupon
        case removeGiftCardCoupon
    }
    ...
    private  var giftCardCoupon: String?
    ...
    var applyGiftCardTxtfld: UITextField?
    var oldQty = ""
    var tagIndex = 0
    
    func removeFromCartEvent(){
        var arrInfo:Array<Dictionary<String,String>> = []
        arrInfo.append(["Count items":"1"])
        EventManager.sharedInstance.eventForPartnerParameter(token: "6x7own", amount: 1.0, revenueCurrency: "EUR", arrDictInfo: arrInfo)
    }
...
//        requstParams["storeId"] = Defaults.storeId
        ...
        case .removeCoupon:
            ...
            verbs = . post
        case .applyGiftCardCoupon:
            apiName = .applyCartGiftCard
            requstParams["giftcardCode"] = giftCardCoupon
            verbs = . post
        case .removeGiftCardCoupon:
            apiName = .removeCartGiftCard
            requstParams["giftcardCode"] = giftCardCoupon
            verbs = . post
        case .updateCart:
            apiName = .updateCart
            ...
            if success == 1 {                
                ...
                if jsonResponse["success"].boolValue == true {
                    ...
                    if jsonResponse["message"].stringValue.removeWhiteSpace.count > 0 {
                        ...
                        if apiName == .updateCart {
                            
                            self?.cartModel.cartProducts[self!.tagIndex].updateQty(qty: self!.oldQty)
                        self?.tagIndex = 0
                        self?.oldQty = ""
    ...
    func doFurtherProcessingWithResult(data: JSON, completion: (Bool) -> Void) {
        ...
        case .getCartData:
            ...
            cartController?.producctCount = cartModel.cartProducts.count
            
            if cartController?.producctCount ?? 0 > 0 {
                cartController?.label.text = "Cart".localized + " (\(cartController?.producctCount ?? 0) " + "Items".localized + ")"
            } else {
                cartController?.label.text = "Cart".localized
            }
        ...
        case .applyCoupon, .removeCoupon, .applyGiftCardCoupon, .removeGiftCardCoupon:
            if data["success"].boolValue {
    ...
    func applyGiftCardCoupon() {
        if let giftCoupon = giftCardCoupon, giftCoupon.count > 0 {
            self.whichApiCall = .applyGiftCardCoupon
            ...
            ShowNotificationMessages.sharedInstance.warningView(message: "Enter Gift Card Code".localized)
        }
    }
    func deleteGiftCardCoupon() {
        self.whichApiCall = .removeGiftCardCoupon
        self.callingHttppApi { (success,jsonData) in
            if success {
                ShowNotificationMessages.sharedInstance.showSuccessSnackBar(message: jsonData["message"].stringValue)
            } else {
    ...
    var arr = ["1", "2", "3", "4", "5", "More".localized]
    func launchPopover(view: UILabel, completion: @escaping (String) -> Void) {
        ...
        selectionMenu.show(style: .popover(sourceView: view, size: CGSize(width: 100, height: 240)), from: cartController!)
...
extension CartViewModel: UITableViewDataSource, UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        if UserDefaults.fetch(key: Defaults.Key.customerToken.rawValue) != nil {
            ...
            case 0:
                if let cell: CartProductTableViewCell = tableView.dequeueReusableCell(with: CartProductTableViewCell.self, for: indexPath) {
                    ...
                    cell.removeBtn.addTapGestureRecognizer {
                        ...
                        let alertController = UIAlertController(title: "Remove From Cart Item".localized, message: "This item will be removed form your cart".localized, preferredStyle: .alert)
                        let okBtn = UIAlertAction(title: "Yes, Remove it".localized, style: .default, handler: {(_ action: UIAlertAction) -> Void in
                            self.removeItemID = self.cartModel.cartProducts[indexPath.row].id
                            self.removeProductFromCart()
                            self.removeFromCartEvent()
                        })
                        ...
                        self.cartController?.navigationController?.present(alertController, animated: true, completion: nil)
                        
                    }
                    cell.wishListClickedclousre = {
                        self.removeItemID = self.cartModel.cartProducts[indexPath.row].id
                        ...
                        if Defaults.customerToken == nil {
                            ...
                            cell.viewContainingController?.navigationController?.pushViewController(customerLoginVC, animated: true)
                        } else {
                    ...
                    cell.qtyView.addTapGestureRecognizer {
                        self.launchPopover(view: cell.qtyLabel) { qty in
                            self.tagIndex = indexPath.row
                            self.oldQty = self.cartModel.cartProducts[indexPath.row].qty
                            self.cartModel.cartProducts[indexPath.row].updateQty(qty: qty)
                            cell.qtyLabel.text = qty
                            cell.lblQtyTxt.text = "Qty".localized + ": "
            ...
            case 3:
                if let cell: GiftCardApplyTableViewCell = tableView.dequeueReusableCell(with: GiftCardApplyTableViewCell.self, for: indexPath) {
                    cell.selectionStyle = .none
                    
                    cell.textField.delegate = self
                    //self.applyGiftCardTxtfld = cell.textField
                    //                if let giftCardCode =  cartModel.giftCardCode {
                    //                    cell.textField.text = giftCardCode
                    //                    cell.textField.isUserInteractionEnabled = false
                    //                    cell.applyBtn.setTitle("Remove".localized, for: .normal)
                    //                    cell.applyBtn.addTapGestureRecognizer {
                    //                        self.giftCardCoupon = cell.textField.text
                    //                        self.deleteGiftCardCoupon()
                    //                    }
                    //                } else {
                    cell.textField.text = ""
                    cell.applyBtn.setTitle("Apply".localized, for: .normal)
            ...
            case 4:
                if let cell: GiftAddedTableViewCell = tableView.dequeueReusableCell(with: GiftAddedTableViewCell.self, for: indexPath) {
                    cell.giftCardList = cartModel.giftData
                    cell.delegate = self
                    cell.selectionStyle = .none
                    return cell
                }
    ...
    func tableView(_ tableView: UITableView, heightForHeaderInSection section: Int) -> CGFloat {
        if section == 0 {
            return  cartModel.banner_img == "" ? 10 : 80
        }
        return 0
    }
    ...
    func tableView(_ tableView: UITableView, viewForHeaderInSection section: Int) -> UIView? {
        if section == 0 {
            let headerView = UIView.init(frame: CGRect.init(x: 0, y: 0, width: tableView.frame.width, height: cartModel.banner_img == "" ? 10 : 80))
            headerView.backgroundColor = .white
           
            if cartModel.banner_img != "" {
                let imageView = UIImageView.init(frame: CGRect.init(x: 0, y: 0, width: headerView.frame.size.width, height: headerView.frame.size.height - 20))
                imageView.setImage(fromURL: cartModel.banner_img ?? "")
...
extension CartViewModel: HeaderViewGiftDelegate {
...
extension CartViewModel: SendRemoveCodeDelegate{
