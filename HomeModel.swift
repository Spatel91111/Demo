...
struct HomeModal {
    ...
    init?(data: JSON) {
       ...
       // UINavigationBar.appearance().barTintColor = AppStaticColors.primaryColor
        ...
        magazine_url = data["magazine_url"].stringValue 
        waitingText = data["waiting_text"].stringValue
        ...
        offerText = data["prefix_text"].stringValue
        suffix_text = data["suffix_text"].stringValue
        clickUrl = data["clickUrl"].stringValue
        ...
        if let brnadsObj = data["ShopByBrands"].arrayObject {
            brands = brnadsObj.map({ (value) -> Brands in
                return Brands(json: JSON(value))
            })
        }
    }
}
